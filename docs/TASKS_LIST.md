...
| Task number | Function name (suggested) | Signature (fixed) | Description |
| :--- | :--- | :--- | :--- |
| Task 4 | initializeSeats | vector<vector<char>> (int, int) | تهيئة مصفوفة مقاعد جديدة |
| Task 5 | displaySeats | void (const vector<vector<char>>&) | عرض خريطة المقاعد |
| Task 6 | isSeatAvailable | bool (const vector<vector<char>>&, int, int) | التحقق من توفر مقعد |
...
| Task 8 | unbookSeat  | bool (vector<vector<char>>&, int, int)| إلغاء حجز مقعد (تحويل X إلى O) |
